 ...
 *
 * $Id: collectDataTable.c,v 1.30 2004/01/16 14:59:03 vijaysp Exp $
 *
 *
...
#ifdef WIN32
#include "../inc/collectDataTable.h"
#else
...
RTC_Get_Time_Frequency (int iToday, collectDataTable_context *pstCtx,
                  unsigned long *pulSTime, unsigned long *pulETime, int *iFreq);

int
RTC_Fetch_Var_Val (oid * name, size_t namelen, u_char *pulNewValue);

void
RTC_Collect_Store_Variables (unsigned int reg, void *list);

void
RTC_callback (unsigned int reg, void *pClientArg);
...
int
RTC_Convert_Date_To_Long (char *pcDate, unsigned long *pulDate);
...
void
RTC_Transfer_File (void *arg);

void
RTC_Thread_Transfer_File (unsigned int uiReg, void *pClientArg);
...
FILE *fpl = NULL;
static RTC_List *gpstHeadList = NULL;
...
unsigned int guiRTCLogFileSendReg = 0;


int giLogFlag = 0;
int giContextChanged = 0;
int giFTProgress = FALSE;
int giRTCFirstTime = 1;

char szSavedLogFile [RTC_FILEPATH_LEN] = { 0 };

/* Temporary log file where variables are collected */
char gszRTCTempLogFile[255] = { 0 };
...
init_collectDataTable(void)
{
    ...
    /* <Start: WIP_NURI_ALL > */
#ifdef WIN32
    pthread_mutex_init(&renamelock, NULL);
#endif
...
RTC_callback (unsigned int reg, void *pClientArg)
{
    ...
    /* Only one thread is allowed to free gpstHeadList at a time */
    ...
    if (iLength == 0)
    {
      ...
      guiRTCReg1 = snmp_alarm_register (RTC_IDLE_TIME,
                                       (int) NULL, RTC_callback, NULL);
      free (stHead);
      return;
    }
...
RTC_Load_Container_From_File ()
{
	...
	/* Loading the collectDataTable rows from file to sub-agent memory */
        while (fread (&stReadCtx, sizeof (collectDataTable_context),1,fp) == 1) 
        {
            ...
            {
                DEBUGMSGTL(("nuri/rtc",
                            "SNMP_MALLOC_TYPEDEF failed errno=%d\n", errno));
                return;
            }
       ...
       /* Check if end of file reached or fread error occurred */
       if (feof (fp) == 0)
       {
           DEBUGMSGTL(("nuri/rtc", "fread failed. errno=%d\n", errno));
       }
                 ...
                 unsigned long *pulSTime, unsigned long *pulETime, int *piFreq)
{
    ...
    switch (iToday)
    {
	    ...
	    /* collectDataSunStartTime and collectDataSunEndTime are
            ...
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataSunStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataSunEndTime);
            *piFreq   = pstCtx->collectDataSunFrequent;
            break;
        case MONDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataMonStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataMonEndTime);
            *piFreq   = pstCtx->collectDataMonFrequent;
            break;
        case TUESDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataTueStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataTueEndTime);
            *piFreq   = pstCtx->collectDataTueFrequent;
            break;
        case WEDNESDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataWedStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataWedEndTime);
            *piFreq   = pstCtx->collectDataWedFrequent;
            break;
        case THURSDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataThuStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataThuEndTime);
            *piFreq   = pstCtx->collectDataThuFrequent;
            break;
        case FRIDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataFriStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataFriEndTime);
            *piFreq   = pstCtx->collectDataFriFrequent;
            break;
        case SATURDAY:
            *pulSTime = CONVERT_TO_SEC (pstCtx->collectDataSatStartTime);
            *pulETime = CONVERT_TO_SEC (pstCtx->collectDataSatEndTime);
            *piFreq   = pstCtx->collectDataSatFrequent;
            break;
        default:
...
Form_Linked_List (RTC_List *pstList, int *iLength)
{
    ...
    while (pstCtx)
    {
        if (RS_IS_ACTIVE (pstCtx->collectDataStatus) == 0)
        {
           DEBUGMSGTL(("nuri/rtc", "Skipping Non-Active Row: "
                       "pstCtx->collectDataIndex=%ld collectDataStatus=%ld\n",
                       pstCtx->collectDataIndex, pstCtx->collectDataStatus));
	...
	/* Is present time within collectDatastartDate and EndDate? */
	    ...
	    /* Get the start time, end time and frequency based on iToday */
            RTC_Get_Time_Frequency (iToday, pstCtx, &pulSTime, &pulETime, &iFreq);
	    ...
	    /* Get current time in seconds from midnight 00:00 */
            ...
            if (ulCTime > pulSTime && ulCTime < pulETime)
            {
                int iCnt = 0;
...
RTC_Convert_Date_To_Long (char *pcDate, unsigned long *pulDate)
{
    ...
    DEBUGMSGTL(("nuri/rtc", "Year=%d Month=%d Day=%d "
                "Hour=%d Min=%d Sec=%d\n", iYear, stTm.tm_mon+1, stTm.tm_mday,
    ...
    /* Assume that mktime will take care of timezone issue */

    *pulDate = mktime(&stTm);

    DEBUGMSGTL(("nuri/rtc", "Date in Long Format=%lu\n", pulDate));
    return 0;
}
	...
	/* Is present time within collectDatastartDate and EndDate? */
	    ...
	    /* Get current time in seconds from midnight 00:00 */
	       ...
	       scheduled execution time and present time. */
            iInitDelay = iFreq - ((ulCTime - pulSTime)%iFreq);
            ...
            if (ulCTime > pulSTime && ulCTime < pulETime)
            {
                DEBUGMSGTL (("nuri/rtc", "if loop: ulCTime=%lu pulSTime=%lu "
                    "pulETime=%lu iCnt=%d\n", ulCTime, pulSTime, pulETime, iCnt));
                ulASTime = iInitDelay + ulCTime;

                if (ulASTime > pulETime)
                {
		...
		/* Calculate the possible number of linked list nodes for
		   current row for the interval specified */
                if (pulETime > iInterval+ulASTime)
                {
                    iCnt = iCnt + iInterval/iFreq;
                }
                else
                {
                    iCnt = iCnt + (int) ((pulETime - ulASTime)/iFreq);
                }
                           ...
                           int *piInterval, int *piLength)
{
    ...
    if (iFreq == 0)
    {
        DEBUGMSGTL(("nuri/rtc", "RTC_Add_Intervals_To_List: "
                    "Skipping Row: Reason: iFreq is ZERO for idx=%lu."
                    "iToday=%d\n", pstCtx->collectDataIndex, iToday));
        return 0;
    }
    ...
    *piLength += 1;
    ...
    /* Assign some unique values to last node */
...
RTC_Within_Period (collectDataTable_context *pstCtx)
{
    ...
    iErr = RTC_Convert_Date_To_Long (pstCtx->collectDataStartDate, &ulSDate);
    if (iErr)
    {
    ...
    iErr = RTC_Convert_Date_To_Long (pstCtx->collectDataEndDate, &ulEDate);
    if (iErr)
    {
...
RTC_Get_Current_Time ()
{
    ...
    ulValue = stTm.tm_hour * 3600 + stTm.tm_min * 60 + stTm.tm_sec;

    DEBUGMSGTL (("nuri/rtc", "Current Time from midnight=%lu\n", ulValue));
    return ulValue;
}
...
void
RTC_Collect_Store_Variables (unsigned int uiReg, void *pClientArg)
{
    int seconds = 0;
    RTC_List *pstList = NULL;
    RTC_List *newhead = NULL;
    
    pstList = (RTC_List *) pClientArg;
...
RTC_Retrieve_Log_Variable (unsigned long ulIndex)
{
    ...
    if (pstFound)
    {
        ...
        DEBUGMSGTL(("nuri/rtc", "Variable: <%s>\n", szVarStr));
    
        memset (ulVal, '\0', sizeof (ulVal));
	...
	/* Check if the LogFile Name has been changed or not.  If changed,
	   Log the details in the new file */
        ...
        {
            giRTCFirstTime = 0;
            guiRTCLogFileSendReg = snmp_alarm_register
                              (glAgentInfoFileTrInterval, (int) NULL,
                               RTC_Thread_Transfer_File, NULL);
        }

        /* Check if LogFile size exceeds the limit or the interval between
           two consecutive File Transfers in seconds is reached. Then,
           Do the File Transfer to agentInfoFileTransferIP and 
           agentInfoFileTransferPortNo */
        ...
        DEBUGMSGTL(("nuri/rtc", "##### glAgentInfoLogFileLimitSize=%ld\n",
                                            glAgentInfoLogFileLimitSize));
      
        if (fdc != -1 && fstat (fdc, &status) == 0)
        {
            /* temp. fix. In case Limit size less than zero */
            if (glAgentInfoLogFileLimitSize < 0)
            {
                glAgentInfoLogFileLimitSize = 10; /* 10 KB */
            }

            /* If file size limit not exceeded return */
            if (status.st_size < (glAgentInfoLogFileLimitSize*1024))
            {
                DEBUGMSGTL(("nuri/rtc", "File size limit not exceeded. "
                             "File Size=<%ld>\n", status.st_size));
            #ifdef WIN32
                pthread_mutex_unlock(&renamelock);
            #endif
            ...
            {
                DEBUGMSGTL(("nuri/rtc", "gcInfoLogFileSystemData is empty!!\n"));
                giLogFlag = 0;
            #ifdef WIN32
                pthread_mutex_unlock(&renamelock);
            #endif
        ...
        {
          DEBUGMSGTL(("nuri/rtc", "Couldn't find stat of <%s> file errno=%d\n",
                      gszRTCTempLogFile, errno));
        #ifdef WIN32
          pthread_mutex_unlock(&renamelock);
        #endif
          return;
        ...
        if (guiRTCLogFileSendReg)
            snmp_alarm_unregister (guiRTCLogFileSendReg);
        guiRTCLogFileSendReg = 0;

        guiRTCLogFileSendReg = snmp_alarm_register (glAgentInfoFileTrInterval,
                               (int) NULL, RTC_Thread_Transfer_File, NULL);
        ...
        /* File size is exceeded. Transferring To Operator...*/
        RTC_Thread_Transfer_File (1, (void *) &iArg);
    }
    else
    {
...
RTC_Transfer_File (void *arg)
{
    ...
    getIPFromASNFormat (gcAgentInfoFileTransferIP, aBuffer);

    sock_addr.sin_addr.s_addr = inet_addr (aBuffer);
...
#ifdef WIN32
    DeleteFile(gcInfoLogFileSystemData);
#endif
    
    /* Move the filename to actual name */
    rename  (gszRTCTempLogFile, gcInfoLogFileSystemData);
#ifdef WIN32
    pthread_mutex_unlock(&renamelock);
#endif

    DEBUGMSGTL(("nuri/rtc", "RTC_Transfer_File: Transferring File <%s>"
                "to IP: <%s> Port: <%ld>\n", gcInfoLogFileSystemData,
                aBuffer, glAgentInfoFileTrPort));

    iErr = FileTrans_SendFile (gcInfoLogFileSystemData, &sock_addr, NULL, 512);

    if (iErr == 1) /* NASC_SUCCESS */
    {
        DEBUGMSGTL(("nuri/rtc", "SendLogFileToManager() Success ....\n"));
    } 
    else 
    {
        DEBUGMSGTL(("nuri/rtc", "SendLogFileToManager() Failed...  iErr=%d.\n",
                                                                        iErr));
    }
    ...
    DEBUGMSGTL(("nuri/rtc", "Waiting for FileTransferIntvl <%ld> seconds...\n",
...
RTC_Thread_Transfer_File (unsigned int uiReg, void *pClientArg)
{
    pthread_t ghRTCPThread;

    if (giFTProgress == FALSE)
    {
        giFTProgress = TRUE;
            
        if (pthread_create (&ghRTCPThread, NULL,
                       (void *) RTC_Transfer_File, (void *) &uiReg) != 0)
        {
            DEBUGMSGTL(("nuri/rtc", "RTC_Transfer_File() Failed!!!\n"));
        }
        else
        {
            DEBUGMSGTL(("nuri/rtc", "RTC_Transfer_File() Success!!!\n"));
        }
    }
    return;
}
...
RTC_Fetch_Var_Val (oid * name, size_t namelen, u_char *pulNewValue)
{
    ...
    {
        DEBUGMSGTL(("nuri/rtc", "RTC_Fetch_Var_Val: API handler function is "
    ...
    if (ucAccess != NULL)
    {
        /* Copy ucAccess based on the type of the variable */
        switch (pstVarPtr->type)
        {
            case ASN_INTEGER:
            case ASN_TIMETICKS:
                lRTCValue = *(long *) ucAccess;
                sprintf (pulNewValue, "%ld", lRTCValue);
                break;
            case ASN_UNSIGNED:
                ulRTCValue = *(u_long *) ucAccess;
                sprintf (pulNewValue, "%lu", ulRTCValue);
                break;
            case ASN_IPADDRESS:
                ulRTCValue = *(u_long *) ucAccess;
                ip_addr.sin_addr.s_addr = ulRTCValue;
                sprintf (pulNewValue, "%s", inet_ntoa (ip_addr.sin_addr));
                break;
            case ASN_OBJECT_ID:
                 for (cnt=0; cnt < var_len; cnt+=4)
                 {
                     lRTCValue = *(long *) (ucAccess+cnt);
                     sprintf (tmp_str, ".%d", lRTCValue);
                     strcat  (str, tmp_str);
                 }
                 strcpy (pulNewValue, str);
                 break;
            case ASN_OCTET_STR:

                /* Use lRTCValue as ASCII flag and set it by default */
                lRTCValue = 1;

                /* Check if string is octet string or display string */
                for (cnt=0; cnt < var_len; cnt++)
                {
                    /* check for non-pritable character */
                    if (*(u_char *) (ucAccess+cnt) < 32 ||
                        *(u_char *) (ucAccess+cnt) > 126)
                    {
                        /* consider it as octet string */
                        lRTCValue = 0; 
                        break;
                    }
                }
                ...
                if (lRTCValue == 1)
                {
                  memcpy (pulNewValue, (u_char *) ucAccess, var_len);
                  break;
                }

                for (cnt=0; cnt < var_len; cnt++)
                {
                    lRTCValue = *(u_char *) (ucAccess+cnt);
                    sprintf (tmp_str, "%02X", lRTCValue);
                    strcat  (str, tmp_str);
                }
                ...
                break;
            default:
               DEBUGMSGTL (("nuri/rtc", "New pstVarPtr->type encountered. "
                            "vars->type=%d\n", pstVarPtr->type));
               /* Assume it as of type int */
               lRTCValue = *(long *) ucAccess;
               sprintf (pulNewValue, "%ld", lRTCValue);
               break;
        }

        DEBUGMSGTL (("nuri/rtc", "type=<%d> pulNewValue=<%s> ucAccess=<%s>"
                     "len=<%d> var_len=<%d>\n", pstVarPtr->type, pulNewValue,
                     (u_long *) ucAccess, strlen (ucAccess), var_len));
...
RTC_Check_LogFile_Name ()
{
    ...
    DEBUGMSGTL(("nuri/rtc", "##### gcInfoLogFileSystemData=%s giLogFlag=%d\n",
                gcInfoLogFileSystemData, giLogFlag));
    DEBUGMSGTL(("nuri/rtc", "##### szSavedLogFile=%s fpl=<%lu>\n",
                                                       szSavedLogFile, fpl));

    if (strlen (gcInfoLogFileSystemData) == 0)
    {
        if (giLogFlag == 0 || giLogFlag == 2)
        {
            if (fpl)
            { 
                fclose (fpl);
    ...
    else if (fpl == NULL || strcmp (gcInfoLogFileSystemData, szSavedLogFile))
    {
        giLogFlag = 2;
        ...
        sprintf (gszRTCTempLogFile, "%s.1", gcInfoLogFileSystemData);

        if ((fpl = fopen (gszRTCTempLogFile, "w")) == (FILE *) NULL)
        {
