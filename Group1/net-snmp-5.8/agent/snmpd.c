...
LPCTSTR         app_name_long = _T("ZZNASCenterAgent");     /* Application Name */
#endif
...
version(void)
{
    printf("\nSmartECO Agent version:  %s\n", netsnmp_get_version());
    printf("Web:               http://www.gkes.co.kr/\n");
    printf("Email:             smarteco@gkes.co.kr\n\n");
}
...
/** modify by D.J Park in 2004.02.09 [START]
 *  modified by HJ.Kwon in 2005.04.18.
 */
#define NURI_KEY   "NuriTelecom"

int GetIPAddr(char *ipaddr)
{
    struct in_addr local_ip_address;
    char slocal[256] = {NULL};
    char *temp = NULL;
    struct hostent *hos;
    ...
    if (gethostname(slocal, sizeof slocal) != -1
      && (hos = gethostbyname(slocal)) != NULL )  {
        memcpy(&local_ip_address, hos->h_addr, 4);
    }
    temp = inet_ntoa(local_ip_address);
    memcpy(ipaddr, temp, strlen(temp));
...
int ConvNumber (char *str, double *dnum )
{
    double num = 0;
    int mul = 1, sign;
    int ii,jj;
...
{
#ifndef WIN32
	char *filename = "/usr/local/conf/ECOAgent_license.lic";
#else
	char *filename = "NasAgent_license.lic";
#endif	
	...
	FILE *fp = NULL;
    	char   temp[64];
    	double firstKey=0, secondKey=0, hwKey=0;
    	double ipKey=0, nuriKey = 0;
    	int  ii = 0, jj;
	int len = 0;

	memset(key,0x00,sizeof(key));
	memset(gen_key,0x00,sizeof(gen_key));
	...
	if( fp == NULL)
	{
		snmp_log(LOG_ERR,"can not open license file[%s] \n",filename);
		exit(1);
	}
	...
	if(len < 1)
	{
		snmp_log(LOG_ERR,"does not exist license key in [%s] \n",filename);
		exit(1);
	}
    ...
    if(ConvNumber(localIP, &ipKey) == 0) {
        snmp_log(LOG_ERR,"HW_Serial Key Generation Fail \n");
        exit(1);
    }

    if(ConvNumber(NURI_KEY, &nuriKey) == 0) {
        snmp_log(LOG_ERR,"HW_Serial Key Generation Fail \n");
        exit(1);
    }

    memset(temp, 0x00, sizeof(temp));
    sprintf(temp, "%20.0lf", nuriKey + ipKey);

    memset(gen_key, 0x00, sizeof(gen_key));
    for(ii = 0, jj = 0; ii < strlen(temp); ii++) {
        if(temp[ii] == ' ') continue;
        if(jj%5 == 0 && jj != 0) {
            gen_key[jj++] = '-';
            gen_key[jj++] = temp[ii];
        } else gen_key[jj++] = temp[ii];
    }

	//snmp_log(6,"IP:%s, GEN_KEY:%s\n",localIP, gen_key);

	if(strcmp(key,gen_key) != 0)
	{
		snmp_log(LOG_ERR,"license is invalid\n");
		exit(1);
	}
	/*
	snmp_log(LOG_ERR,"license is ok key[%s] gen_key[%s]\n",key,gen_key);
         */
}
...
#endif
{
     ...
     */
    snmp_log(LOG_INFO, "SmartECO Agent version %s\n", netsnmp_get_version());
#ifdef WIN32SERVICE
...
receive(void)
{
    ...
    while (netsnmp_running) {
	    ...
	    snmp_log(LOG_INFO, "SmartECO Agent version %s restarted\n",
		     netsnmp_get_version());
            update_config();
...
_tmain(int argc, TCHAR * argv[])
{
    ...
    LPCTSTR         lpszServiceDisplayName = _T("NASCenterAgent Daemon");       /* Display Name */
    LPCTSTR         lpszServiceDescription = _T("NASCenterAgent for windows from NuriTelecom");
...
BOOL ChangeExecuteDir(TCHAR* changedDir)
{
    TCHAR fileName[MAX_PATH * 2] = { 0, };
    TCHAR filePath[MAX_PATH * 2] = { 0, };
    BOOL bResult = FALSE;
    int len, i;
    GetModuleFileName(NULL, (TCHAR*)&fileName, sizeof(fileName) * sizeof(TCHAR));
    ...
    if (i > 0)
    {
        _tcsncpy(filePath, fileName, i + 1);
        if (changedDir != NULL)
            _tcscpy(changedDir, filePath);
        if (_tchdir(filePath) == 0)
        {
            bResult = TRUE;
        }
