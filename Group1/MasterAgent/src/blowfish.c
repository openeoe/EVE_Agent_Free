...
void NAS_Encrypt(BLOWFISH_context *bctx, char *outbuff,
				char *inbuff) {
	int i,q,n;
	char *cPtr;
	...
	n = (size/8)*8;
	n+=8;
	for(i=strlen(inbuff);i<n;i++){
		inbuff[i] = 0;
	}
	for(i=0;i<n;i+=8){
		encrypt_block(bctx,&outbuff[i],&inbuff[i]);	
	}
}


void NAS_Decrypt(BLOWFISH_context *bctx, char *outbuff,
				char *inbuff) {
	int i=0;
	while(i<MAX_MSG_SIZE){
		decrypt_block(bctx,&outbuff[i],&inbuff[i]);
		if(inbuff[i+8-1]==0){
			break;
		}
		i+=8;
...
/*main() {
	...
	unsigned char data[] = "testBlockdsfdsfx\0\0\0\0\0\0\0\0";
	unsigned char buff[1024]; 
	unsigned char buff1[1024];
	bf_setkey(&bctx,"TESTKEY",7);
	NAS_Encrypt(&bctx,buff,data);
	NAS_Decrypt(&bctx,buff1,buff);
	printf("buff1 %s\n",buff1);
	printf("Length %d\n",strlen(buff1));
}*/
